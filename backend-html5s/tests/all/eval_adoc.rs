...
assert_inline_html!(passthrough, "+_<foo>&_+", r#"<p>_&lt;foo&gt;&amp;_</p>"#);
assert_inline_html!(
  text_span,
  "[.foo]#bar#",
  r#"<p><span class="foo">bar</span></p>"#
);
assert_inline_html!(passthrough_block, "[pass]\n_<foo>&_", "_<foo>&_");
assert_inline_html!(highlight, "foo #bar#", r#"<p>foo <mark>bar</mark></p>"#);
assert_inline_html!(mono, "foo `bar`", r#"<p>foo <code>bar</code></p>"#);
assert_inline_html!(
  passthrough_2,
  "rofl +_foo_+ lol",
  r#"<p>rofl _foo_ lol</p>"#
);
assert_inline_html!(
  inline_passthrough,
  "+++_<foo>&_+++ bar",
...
assert_inline_html!(
  subscript,
  "foo ~bar~ baz",
  r#"<p>foo <sub>bar</sub> baz</p>"#
);
assert_inline_html!(
  superscript,
  "foo ^bar^ baz",
  r#"<p>foo <sup>bar</sup> baz</p>"#
);
assert_inline_html!(
  not_quotes,
  "foo `'bar'`",
...
assert_inline_html!(multichar_whitespace, "foo   bar", r#"<p>foo   bar</p>"#);
assert_inline_html!(
  litmono_attr_ref,
  "`+{name}+`",
  r#"<p><code>{name}</code></p>"#
);
assert_inline_html!(not_implicit_apostrophe, "('foo')", r#"<p>('foo')</p>"#);

assert_inline_html!(
  email_w_dot,
  "email a.b@c.com",
  r#"<p>email <a href="mailto:a.b@c.com">a.b@c.com</a></p>"#
);
...
assert_inline_html!(
  ...
  //     v----v -- inline passthru
  "foo `++a`b`++`",
  r#"<p>foo <code>a`b`</code></p>"#
);

assert_inline_html!(
  not_passthrough,
  "`\\d+[a]\\d+[b]`",
  "<p><code>\\d+[a]\\d+[b]</code></p>"
);
...
assert_inline_html!(
  confusing_combo,
  "`*` foo `*`",
  r#"<p><code><strong></code> foo <code></strong></code></p>"#
);
...
assert_inline_html!(
  ...
  r#"<p>John&#8217;s Hideout is the Whites&#8217; place&#8230;&#8203; foo'bar</p>"#
);

// COMMENTED OUT: btn macro not supported in jirutka backend
// assert_inline_html!(
//   btn_macro,
...
assert_html!(
  ...
  html! {r#"
    ...
    <p>foo // not a comment
    bar</p>
    <div class="listing-block"><pre>// retained in verbatim</pre></div>
  "#}
);
